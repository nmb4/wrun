...
if (events.count == 0 || matched == null) {
    System.print("SKIP: native watcher poll mode strict probe (no native content event observed)")
    if (File.exists(path)) File.delete(path)
    Process.exit(0)
}
...
System.print("PASS: native watcher poll mode smoke test (strict native)")
